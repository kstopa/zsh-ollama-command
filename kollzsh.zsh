...
validate_required() {
  ...
  if ! curl -s "${KOLLZSH_URL}/api/tags" | grep -q $KOLLZSH_MODEL; then
    echo "🚨: zsh-ollama-command failed as model ${KOLLZSH_MODEL} server NOT found!"
    echo "Please start it with 'ollama pull ${KOLLZSH_MODEL}' or adjust KOLLZSH_MODEL"
    return 1;
  fi
}
...
# Function to validate JSON and clean it up
validate_json() {
  local input="$1"
  # Remove any null bytes and clean up the input
  cleaned_input=$(echo "$input" | tr -d '\0' | sed 's/\r//g')
  # Log the input and cleaned input for debugging
  log_debug "Original JSON input:" "$input"
  log_debug "Cleaned JSON input:" "$cleaned_input"
  
  # Check if it's valid JSON
  if echo "$cleaned_input" | jq '.' >/dev/null 2>&1; then
    echo "$cleaned_input"
    return 0
  else
